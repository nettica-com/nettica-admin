<template>
    <v-container style="padding-top:0px">
        ...
        <v-card>
            ...
            <v-data-table v-if="listView" :headers="headers" :items="hosts" :search="search" :items-per-page="25"
                no-data-text="No Hosts.  Click above to create your first host, or use Nettica Agent on the host."
                no-results-text="No results matching your search" :footer-props="footerProps">
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-form ref="form" v-model="valid">
                                <v-text-field v-model="host.name" label="Host friendly name"
                                    :rules="[v => !!v || 'host name is required',]" required />
                                <v-select return-object v-model="netList.selected" :items="netList.items" item-text="text"
                                    item-value="value" label="Join this net" :rules="[v => !!v || 'Net is required',]"
                                ...
                                <v-combobox v-model="host.tags" chips hint="Enter a tag, hit tab, hit enter." label="Tags"
                                    multiple dark>
                                ...
                                <v-switch v-model="host.enable" color="success" inset
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-form ref="form" v-model="valid">
                                <v-text-field v-model="host.name" label="Friendly name"
                                    :rules="[v => !!v || 'host name is required',]" required />
                                ...
                                <v-text-field v-model="host.current.endpoint" label="Public endpoint for clients" />
                                <v-text-field v-model="host.current.listenPort" type="number" label="Listen port" />
                                <v-combobox v-model="host.tags" chips hint="Write tag name and hit enter" label="Tags"
                                    multiple dark>
                                    <template v-slot:selection="{ attrs, item, select, selected }">
                                        <v-chip v-bind="attrs" :input-value="selected" close @click="select"
                                            @click:close="host.tags.splice(host.tags.indexOf(item), 1)">
                        ...
                        <div class="d-flex flex-no-wrap justify-space-between">
                            <v-col cols="12">
                                <v-select return-object v-model="platforms.selected" :items="platforms.items"
                                    item-text="text" item-value="value" label="Platform of this host" single
                                    persistent-hint />

                                <v-combobox v-model="host.current.address" chips
                                ...
                                <v-combobox v-model="host.current.allowedIPs" chips
                                ...
                                <v-combobox v-model="host.current.dns" chips
                                    hint="Enter IP address(es) and hit enter or leave empty."
                                    label="DNS servers for this host" multiple dark>
                                    <template v-slot:selection="{ attrs, item, select, selected }">
                                ...
                                <v-text-field v-model="host.current.privateKey" label="Private key" autocomplete="off"
                                ...
                                <v-text-field v-model="host.current.presharedKey" label="Preshared Key" autocomplete="off"
                                ...
                                <v-text-field type="number" v-model="host.current.mtu" label="Define global MTU"
                                    hint="Leave at 0 and let us take care of MTU" />
                                <v-text-field type="number" v-model="host.current.persistentKeepalive"
                                    label="Persistent keepalive"
                                    hint="To disable, set to 0.  Recommended value 29 (seconds)" />
                                <v-textarea v-model="host.current.postUp" label="PostUp Script"
                                    hint="Only applies to linux servers" />
                                <v-textarea v-model="host.current.postDown" label="PostDown Script"
                                    hint="Only applies to linux servers" />
                                <v-switch v-model="host.current.subnetRouting" color="success" inset
                                    label="Enable subnet routing" />
                                <v-switch v-model="host.current.upnp" color="success" inset label="Enable UPnP" />
                                <v-switch v-model="host.current.enableDns" color="success" inset
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-form ref="form" v-model="valid">
                                <v-text-field v-model="host.name" label="New name for host"
                                    :rules="[v => !!v || 'host name is required',]" required />

                                <v-select return-object v-model="netList.selected" :items="netList.items" item-text="text"
                                    item-value="value" label="Copy to this net" :rules="[v => !!v || 'Net is required',]"
                                    single persistent-hint required />
                            </v-form>
                        </v-col>
                    </v-row>
...
export default {
    ...
    methods: {
        ...
        startUpdate(host) {
            ...
            this.netList = {
                selected: { "text": this.host.netName, "value": this.host.netid },
                items: []
            }
        ...
        update(host) {
            ...
            if (this.publicSubnets) {
                this.host.current.allowedIPs.push("0.0.0.0/5", "8.0.0.0/7", "11.0.0.0/8", "12.0.0.0/6", "16.0.0.0/4", "32.0.0.0/3", "64.0.0.0/3",
