<template>
    <v-container>
        ...
        <v-card>
            ...
            <v-data-table v-if="listView" :headers="headers" :items="accounts" :search="search">
                ...
                <template v-slot:item.action="{ item }">
                    <v-row>
                        <v-icon class="pr-1 pl-1" @click.stop="startUpdate(item)">
                            mdi-account-switch
                        </v-icon>
                        <v-icon class="pr-1 pl-1" @click.stop="startUpdate(item)">
                            mdi-square-edit-outline
                        </v-icon>
                        <v-icon class="pr-1 pl-1" @click="remove(item)">
            ...
            <v-card-text v-else>
                <v-row>
                    <v-col v-for="(user, i) in users " :key="i" sm12 lg6>
                        <v-card :color="user.enable ? '#1F7087' : 'warning'" class="mx-auto" raised shaped>
                            <v-list-item>
                                <v-list-item-content>
                            ...
                            <v-card-actions>
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn text v-on:click="forceFileDownload(user)" v-on="on">
                                ...
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn text @click.stop="startUpdate(user)" v-on="on">
                                ...
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn text @click="remove(user)" v-on="on">
                                ...
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <v-btn text @click="email(user)" v-on="on">
                                ...
                                <v-tooltip right>
                                    <template v-slot:activator="{ on }">
                                        <v-switch dark v-on="on" color="success" v-model="user.enable"
        ...
        <v-dialog v-if="user" v-model="dialogCreate" max-width="550">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer />
                    <v-btn :disabled="!valid" color="success" @click="email(toAddress)">
                        Submit
